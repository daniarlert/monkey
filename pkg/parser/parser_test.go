...
func TestIntegerLiteralExpression(t *testing.T) {
	...
	if literal.Value != 5 {
		t.Errorf("ident.Value not %v. got=%v", 5, literal.Value)
	}

	if literal.TokenLiteral() != "5" {
		t.Errorf("ident.TokenLiteral not %v. got=%s", 5, literal.TokenLiteral())
	}
}
...
func testIntegerLiteral(t *testing.T, i ast.Expression, value int64) bool {
	...
	if !ok {
		t.Errorf("i not *ast.IntegerLigeral. got=%T", i)
		return false
	}
